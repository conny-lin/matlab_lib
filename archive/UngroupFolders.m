function [Time] = UngroupFolders(pExp,pFun)

[Expfdat,Groupf,pGroupf] = importgroupedExpraw2('*.dat',pExp,'MatLabAnalysis');
[T,MWTf,pMWTf] = getgroupedMWTf2(pGroupf); % get MWTf
[MWTftrv,MWTfdat] = importMWTdattrv(MWTf,pMWTf,T);
...
function [T,MWTf,pMWTf] = getgroupedMWTf2(pGroupf)
...
%% set groups
 function [GA,GAA,MWTfgcode] = setgroup(MWTfdat,pFun,pExp,id)
    ...
    [GA] = groupnameswitchboard4(MWTfgcode,MWTfdat,gcode,RCpart,pFun,pExp,0); % assign group name using group files 
...
RCpart = A;
        ...
        function [GA] = groupnameswitchboard4(MWTfgcode,MWTfdat,gcode,RCpart,pFun,pExp,id)
        % assign group names
        ...
        switch q1;
            case 1 % select assign (by continueing)
                ...
                [GA] = manualassigngroupname(MWTfgcode);
        ...
        if input('MWT file naming correct? (y=1,n=0): ')==0; % ask if the MWTf naming was incorrect
            ...
            function correctMWTnamingswitchboard2(MWTfdat,pExp,pFun)
    ...
    if input('only group code incorect? (y=1,n=0): ')==1; 
        ...
        for x = 1:size(fpart,1);
            ...
            %end
                    ...
                    function [filenamelist,filepathlist] = ...
                        getallfilesinallgroupedMWTfolders(pExp)
...
function [MWTftrv,MWTfdat,MWTfdatG,MWTftrvG] = groupMWTimport3(MWTfdat,...
    MWTftrv,MWTfgcode)
...
[Graph] = figvprep4(Stats.curve,'MWTfdata',GAA,Xn,Yn,...
...
switch id
    ...
    function [Graph] = figvprep4(D,fn,GAA,Yn,Xn,pFun,pExp,setfilename)
%% Graphing: individual
...
Graph.Ylabel = Yn;
Graph.Xlabel = Xn;
%% [BUG] Can't graph X axis Graph.X = X';
