function correctMWTfgcodeungrouped(MWTfdatA,pMWTfA,gn)
%% [FUTURE DEVELOPMENT] layer = how many layers of folder before reaching MWT folder
% only works with ungrouped files


%disp(MWTfdatA(:,2)); % display all experiment names
[RCpart] = parseRCname1(MWTfdatA); % ask if only group codes are inccorectto reassign
[f,pf] = getallfilesinallungroupedMWTfolders(pMWTfA);
