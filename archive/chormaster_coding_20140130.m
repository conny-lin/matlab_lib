function [chorscript] = chormaster_coding_20140130(path,option)
%function [chorscript] = chormaster(path,option)
%% instruction
% [chorscript] = chormaster(path,analysisoption)
...
switch Chor
    case 'LadyGaGa'
    choroutput = [javacall,b,javaRAM,b,chor,b,pixelsize,b,speed,b,mintime,...
...
%% eval MWT content to see if already chor-ed
regexp(chorscript,'-O ')
regexp(chorscript,'-O ')
...
% if more than one paths and paths are cell array
