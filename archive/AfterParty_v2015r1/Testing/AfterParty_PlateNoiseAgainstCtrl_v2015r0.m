function AFR = AfterParty_PlateNoiseAgainstCtrl_v2015r0(AFR)
...
CG = chooseoption(GU,2);


%% for each expdate
% calculate mean for each exp
pM = MWTSet.MWTInfo.pMWT;
MWTfnAll = mwtpath_parse(pM,{'MWTname'});

%% take experiment % diff (mwt/exp mean within group)
msr = {'RevFreq'; 'RevDur';'RevSpeed'};
...
D = DataG.(g{gi}).(msr{msri});
mwtn = D.pMWT;
D.MWTfn = mwtn;
[i,j] = ismember(MWTfnAll,mwtn);
pm = pM(j(i));
D.pMWT = pm;
en = mwtpath_parse(pm,{'expname'});
enu = unique(en);

A = nan(size(D.mean));
B = nan(size(D.mean,1),numel(enu));

for enui = 1:numel(enu)
i = ismember(en,enu{enui});
d = D.mean(:,i);
expMean = mean(d,2);
A(:,i) = d./repmat(expMean,1,size(d,2));
B(:,enui) = expMean;
end
D.meanStd2exp  = A;
D.meanExp  = B;

DataG.(g{gi}).(msr{msri}) = D;
end
end
