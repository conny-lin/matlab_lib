...
%% GRAPHING: INITIAL & LAST 3 TAPS SEPARATE BAR GRAPHS
M = fieldnames(MWTSet.Graph);
...
for m = 1:numel(M)
    ...
    savefigeps(['Initial vs hab level',M{m}],pSaveA);
end
...
%     legend1 = legend(axes1,'show');
%     set(legend1,'Location','EastOutside','EdgeColor',[1 1 1],'YColor',[1 1 1],'XColor',[1 1 1]);
    savefigeps(['Habituation stength ',M{m}],pSaveA);
...
savefigeps(['Habituation stength percent difference',M{m}],pSaveA);
