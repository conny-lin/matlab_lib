function [chorscript] = chormaster(mwtpath,option,pProgram)
...
AnaList = '(LadyGaGa)|(ShaneSpark)|(Beethoven)|(DrunkPosture)|(AnnaPavlova)|(Rastor)';
...
% beethoven 
% beethoven = '''/Users/connylinlin/Documents/Java_Programs/Beethoven_v2.jar''';
beethovenversion = 'Beethoven_v2.jar';
beethoven = ['''',pJava,'/',beethovenversion,''''];

% chor 
% chore = '''/Users/connylinlin/Documents/Java_Programs/Chore_1.3.0.r1035.jar'''
chorversion = 'Chore_1.3.0.r1035.jar';
chor = ['''',pJava,'/',chorversion,''''];
...
if size(mwtpath,1)>1 && iscell(mwtpath)==1;
    if isempty(regexp(mwtpath{1},'\<(\d{8})[_](\d{6})\>','once'))==0;
        i = not(cellfun(@isempty,regexp(mwtpath,'\<(\d{8})[_](\d{6})\>')));
        k = find(i);
        for x = 1:sum(i); 
            [~,fn] = fileparts(mwtpath{k(x)});
            file = strcat('''',mwtpath{k(x)},''''); 
            display(sprintf(str,fn));
            system([chorscript file], '-echo');       
        end
        
    % if input is a list of exp folder    
    elseif isempty(regexp(mwtpath{1},'\<(\d{8})[_](\d{6})\>','once'))==1;
        for x = 1:size(mwtpath,1); 
            [~,pMWT] = dircontentmwt(mwtpath{x});
            for y = 1:numel(pMWT)
            [~,fn] = fileparts(pMWT{y});
            file = strcat('''',pMWT{x},''''); 
            display(sprintf(str,fn));
...
elseif ischar(mwtpath)==1 && size(mwtpath,1)==1 && isempty(regexp(mwtpath,'\<(\d{8})[_](\d{6})\>','once'))==0;
        file = strcat('''',mwtpath{1},''''); 
        display(sprintf(str,fn));
        system([chorscript file], '-echo');

% if input might be a home foler to Exp folders    
elseif ischar(mwtpath) ==1 && isempty(regexp(mwtpath,'(\d{8})[_](\d{6})','once'))==1 && ...
        size(mwtpath,1)==1;
    [~,~,~,~,pMWTf,fn,~] = getMWTpathsNname(mwtpath,'noshow');
