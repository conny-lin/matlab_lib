...
%% VARAGIN requirement
% ReqInput = {'MWTfG';'chorfile';'pSaveA'};
% switch nargin
%     case 0
...
%% [moved to chor master] STEP3E: EXCLUDE CHOR PROBLEM MWT FILES FROM ANALYSIS
...
Graph = B;
% 
%% [Moved] STEP4B: PREPARE GRAPH SUMMARY FOR SUBPLOTS
% Graph = [];
...
% GraphA = [];
% for m = 1:numel(M)
%     GraphA.(M{m}).X = Graph.X.(M{m});
%     GraphA.(M{m}).Y = Graph.Y.(M{m});
%     GraphA.(M{m}).E = Graph.E.(M{m});
% end
