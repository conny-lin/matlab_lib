% function [varargout] = chormaster2(MWTSet,varargin)
%% UPDATES
    % 20140413
...
switch MWTSet.chor.chorstatus
    case 'old'
        pA = MWTSet.PATHS.pAnalysis;
        ...
        if isempty(p)==0
            ...
            pMWTA = p;   
            [~,MWTfnA] = cellfun(@fileparts,pMWTA,'UniformOutput',0);

            [i,j] = ismember(MWTSet.MWTInfo.MWTfn,MWTfnA);
            val = zeros(size(pMWT));
            val(i) = 1;
            k = find(i);
            j = j(i);
            ...
            for m = 1:numel(j)
                ...
                for f = 1:numel(fval)
                    i = dircontent(pMWTA{j(m)},fval{f});
                    if isempty(i) == 0;
                        val(k(m)) = val(k(m))+1;
                    end
                end
            end        
            val = val ~= numel(fval)+1;
            pMWTc = pMWT(val);
        else
            pMWTc = pMWT;
        end
...
%% RUN CHOR
if isempty(pMWTc) == 0
    display(sprintf('Choring %d MWT files:',numel(pMWTc)));    
    for x = 1:numel(pMWTc); 
        % report status
        [~,fn] = fileparts(pMWTc{x}); 
        display(sprintf('Chor-ing %d/%d MWTfile [%s]...',...
            x,numel(pMWTc),fn));
        % unzip at analysis archive location
        p = pMWTc{x};
        p2 = regexprep(p,MWTSet.PATHS.pData,MWTSet.PATHS.pAnalysis);
        d = regexprep(p2,'.zip','');
        if isdir(d) == 0; mkdir(d); end
        unzip(p,fileparts(d))   
        % run chor
        file = strcat('''',d,''''); 
        ...
        [~,p] = dircontent(d);
        i = regexpcellout(p,'(.blobs)|(.png)|(.summary)|(.set)');
        cellfun(@delete,p(i));
...
%% GENERAL CHOR JAVA SYNTAX (chorescript)
...
if sum(strcmp(A,'Setting')) == 1
    MWTSet = A{1};
    [MWTSet] = chormaster_setting(MWTSet);
    varargout{1} = MWTSet;
    return
elseif sum(strcmp(A,'Legend')) == 1
    [MWTSet] = chormaster_legend(MWTSet);
    varargout{1} = MWTSet;
    return
end
...
%% Check MWT folders
% store away original MWTfG
MWTSet.MWTfG_Old = MWTSet.MWTfG;
...
% validate MWT file name (take out problem folders)
...
% CHECK CHOR HISTORY
% chorstatus = MWTSet.chorstatus;
switch MWTSet.chorstatus
    case 'old'
        [pMWTp,pMWTc] = checkchorhistory(pMWT,MWTSet.chorfile);
    case 'new'
        pMWTc = pMWT;
end
...
%%
if isempty(pMWTc) == 0
%     runchor(pMWTc,MWTSet.chorscript); % capable of running multiple chorscript
    display(sprintf('Choring %d MWT files:',numel(pMWTc)));
...
%% RE-CHECK CHOR OUTPUT
[pMWTpass,pMWTfail] = checkchorhistory(pMWT,MWTSet.chorfile);
[MWTfG] = reconstructMWTfG(pMWTpass);




%% VARARGOUT
display 'preping output';
MWTSet.pMWTchorfail = pMWTfail;
MWTSet.pMWTchorpass = pMWTpass;
MWTSet.pMWT = pMWT;
% MWTSet.MWTBadFiles = MWTBadFiles;
MWTSet.MWTfG = MWTfG;
if isstruct(MWTSet) ~=1
    error('problem with MWTSet');
end
