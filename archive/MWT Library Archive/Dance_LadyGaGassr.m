...
for e = 1:size(pExpfT,1)
pExp = pExpfT{e};
display ' ';
[~,expname] = fileparts(pExp);
display('Importing ssr generated by Choreography...');
str = 'from [%s]';
...
%% import and summarize MWTfssr 
% creating MWTftrv legend
MWTfssrL = {1,'MWTfile name';2,'Run name';3,'RawData';4,'NReversed';...
...
%         end
    ...
    MWTfssr(x,1:(size(B,2))) = B;
    MWTfssr(x,:) = B;
end


% saving
[savename] = creatematsavename(pExp,'LadyGaGassr','.mat'); % create save name
cd(pExp);
save(savename);
display('done');
%% Data processing
[MWTfssrG] = GroupData(MWTfssr,pExp);
% replace group code with gname
%%
LadyGaGaSubPlot(MWTfssrG,pExp,'LadyGaGassr')
cd(pExp);
save('LadyGaGassr.mat','MWTfssrG','MWTfssr');
end
end
