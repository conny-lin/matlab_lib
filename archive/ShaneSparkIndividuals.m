function ShaneSparkIndividuals(pExpfD)
%% loop for each exp
%% problem file
% expProblem = [expProblem;expX];
% expI = expX+1;  
expI = 1;
%%
for expX = expI:numel(pExpfD)
Expfn = ExpfnD{expX};
pExpf = pExpfD{expX};
%% STEP4C: Make Output folder
...
%% Get single exp
% [O] = MWTDataBaseMaster(pExpfT,'GetSingleExpInfo');
% GfnT = O.GfnT; pGfT = O.pGfT; pMWTfT = O.pMWTfT; MWTfnT = O.MWTfnT;

[~,~,GfnT,pGfT] = dircontent(pExpf);
i = ismember(GfnT,{'MatlabAnalysis'});
GfnT(i) = []; pGfT(i) = [];
...
for g = 1:numel(gnamechose);
    [MWTfn,pMWTf] = dircontentmwt(pGfT{g});
