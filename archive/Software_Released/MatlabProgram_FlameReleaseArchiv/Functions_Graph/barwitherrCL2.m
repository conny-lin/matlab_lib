...
function varargout = barwitherrCL2(errors,varargin)
%% Function
...
handles.bar = bar(varargin{:},'FaceColor',[0.55 0.72 0.95],...
    'EdgeColor',[1 1 1]); % standard implementation of bar fn
...
if nRows > 1
    ...
    for col = 1:nCols
        ...
        hErrorbar(col) = errorbar(mean(x,1), values(xOrder,col), ...
            lowerErrors(xOrder,col), upperErrors(xOrder, col), '.k',...
            'LineStyle','none','Color',[0.078 0.168 0.55]);
...
% Create xlabel
xlabel('Age','FontSize',18,'FontName','Calibri');

% Create ylabel
ylabel('Score Improvement from previous session','FontSize',18,...
    'FontName','Calibri');
hold off
