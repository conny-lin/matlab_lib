function [chorscript] = chormaster(pMWT,varargin)
...
%% PATH
%% for flame
javapath = '/Volumes/FLAME/MatlabProgram/MATLAB MWT/SubFun_Java';
...
%     else
%         
%     % check if Flame is attached
%         isdir(paths.pFlame
%          'Rose is not attached';
    end
...
%% VALIDATE PATHS INTPUTS
% validate paths = cell array & = MWT folders
[~,f] = cellfun(@fileparts,pMWT,'UniformOutput',0);
if iscell(pMWT)==1 && isempty(regexp(f,'\<(\d{8})[_](\d{6})\>','once'))==0
else
    error 'incorrect paths input';
end
