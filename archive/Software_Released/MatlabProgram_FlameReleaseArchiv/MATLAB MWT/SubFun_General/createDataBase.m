function [MWTDatabase,A] = createDataBase(Home)
%% create database
...
[ph,fn] = cellfun(@fileparts,allpaths,'UniformOutput',0); % get filename of all paths
for x = 1:numel(pExpV);
    [gn,pg] = dircontentext(pExpV{x},'Groups_*.mat');
    if isempty(gn) ==1;
        gn = {};
        gfp = {};
    end
...
for x = 1:numel(pExpV);
load(gfp{x},'Groups')
[expname] = cellfunexpr(Groups,Expfn{x});
 info = [expname Groups];
 GroupDataBase = [GroupDataBase;info];
end
...
% create output
i = ismember(expqurey,Expfn);
pExpM = pExpV(matchexp);
