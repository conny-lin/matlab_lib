...
writetable(ST,fullfile(datapath,'plausible vs videotext anova.csv'),'WriteRowNames',1);
writetable(T,fullfile(datapath,'plausible vs videotext descriptive.csv'));

return
% ... vs voting
prefix = 'plausible vs videotext voting';
[sout,StatsTable] = anovan_std(Plausible,{VT,Voting},{'VT','Voting'},datapath,'prefix',[prefix,' ']);
writetable(StatsTable,fullfile(datapath,[prefix,'.csv']))
% ... vs sex
prefix = 'plausible vs videotext sex';
[sout,StatsTable] = anovan_std(Plausible,{VT,Sex},{'VT','Sex'},datapath,'prefix',[prefix,' ']);
writetable(StatsTable,fullfile(datapath,[prefix,'.csv']));
% ... vs media
prefix = 'plausible vs videotext media';
[sout,StatsTable] = anovan_std(Plausible,{VT,Media},{'VT','Media'},datapath,'prefix',[prefix,' ']);
writetable(StatsTable,fullfile(datapath,[prefix,'.csv']));
% ... vs voting, sex, media
prefix = 'plausible vs videotext voting sex media';
[sout,StatsTable] = anovan_std(Plausible,{VT,Sex, Voting, Media},{'VT','Sex','Voting','Media'},datapath,'prefix',[prefix,' ']);
writetable(StatsTable,fullfile(datapath,[prefix,'.csv']));
