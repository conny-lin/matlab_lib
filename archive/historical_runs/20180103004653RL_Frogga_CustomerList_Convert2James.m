...
for ci = 1:numel(pcustomerlist)
    ...
    filepath = fullfile(pData,filename);
    if ~isdir(filepath) 
        if ~strcmp(filename,'non-universities.csv')
            D = importfile_froggacustomerlist(filepath);
        end
...
%% ODD LIST IMPORT LIST AND CONVERT %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% get file name
filename = 'nonuniversities.mat';
filepath = fullfile(pM,filename);
load(filepath)
D = nonuniversities;
% convert to james format
D1 = D(:,{'Institution','Floor','Room','PI','Labcontact','Email','Phone','Leads','ReferenceEquipmentReference'});
% fill room number as ? if empty
room = D1.Room;
room(cellfun(@isempty,D1.Room)) = {'?'};
% get room number first digit
...
% save as adjusted list
savename = fullfile(pM,'nonuniversities.csv');
writetable(D3,savename)
