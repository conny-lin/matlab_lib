...
Score = import_TLAscore2(pf,[2017 2 1]); % import
% --------------------------------
...
c = ClassInfo.class;
c = num2cellstr(c);
a = repmat({'pct by week class'},numel(id),1);
s = repmat({' '},numel(id),1);
b = repmat({'pid'},numel(id),1);
e = repmat({'.csv'},numel(id),1);
fnm = strjoinrows([a c s b id e],'');

fn = dircontent(pSave);
[i,j] = ismember(fnm,fn);
if sum(i) ~= numel(id)
   fprintf('below id did not have data\n');
   fnm(~i)
end
