...
% -------------------------------------------------------------------------
% get genotype info | 20170819
load('/Volumes/COBOLT/MWT/MWTDB.mat'); % load allele 
StrainList = MWTDB.strain;
% make table
T = table;
T.group = groupnames;
T.gene = genenames;
T.strain = strainnames;
T = outerjoin(T, StrainList(:,{'strain','genotype'}),'MergeKeys',1,'Type','left');
TALL = T;
% -------------------------------------------------------------------------
...
colnames = {'curve_file_source','curve_N',...
            ...
            'curve_wt_pct_ttest','curve_mt_pct_ttest',...
            'curve_wt_pct_tstat','curve_mt_pct_tstat',...
            'curve_pct_anova','curve_pct_anovastr','curve_pct_es'};
coltype = cellfunexpr(colnames','double');
coltype([1:2 9:10 12]) = {'cell'};
...
for si = 1:numel(strainnames)
    ...
    for ci = 1:numel(colnames)
        ...
        switch cn
            ...
            case 'curve_mt_pct_ttest'
                a = TTEST.pv(ismember(TTEST.gn,[sn,'_400mM']));
            case 'curve_wt_pct_tstat'
                a = TTEST.tstat(ismember(TTEST.gn,'N2_400mM'));
            case 'curve_mt_pct_tstat'
                a = TTEST.tstat(ismember(TTEST.gn,[sn,'_400mM']));
            case 'curve_pct_anova'
                a = ANOVA.p('Groups');
            case 'curve_pct_anovastr'
                t = ANOVA;
                a = anova_textresult(t);
            case 'curve_pct_es'
                a = ES;
        end
        % put in table
        if strcmp(coltype{ci},'cell')
            T.(cn){si} = a;
        else
            T.(cn)(si) = a;
        end
...
T = table;
T.strain = strainnames;
colnames = {'rev_file_source','rev_c_N',...
...
for si = 1:numel(strainnames)
    ...
    for ci = 1:numel(colnames)
        ...
        switch cn
            ...
            case 'rev_c_manovastr'
                b = anovan_textresult(RM);
                b = regexprep(b,'[*]tap|tap','');
                b = regexp(b,'\n','split');
                a = strjoin(b,', ');
            case 'rev_c_es'
                a = ES;
            ...
            case 'rev_c_ph_w4m4'
                ...
                a = PHC.pValue(i);
            case 'rev_c_ph_m0m4'
                i = ismember(PHC.factors_1,[sn,'*0']) & ismember(PHC.factors_2,[sn,'*400']) | ...
                    ismember(PHC.factors_2,[sn,'*0']) & ismember(PHC.factors_1,[sn,'*400']);
                a = PHC.pValue(i);
            case 'rev_c_spw_w0w4'
            ...
            case 'rev_c_spw_m0m4'
                a = timecomppertap(PHCbytap,[sn,'*0'],[sn,'*400']);
...
for si = 1:numel(strainnames)
    ...
    if ~isfield(MWTSet.Stats.AccProb,'rmanova')
    ...
    for ci = cnlist
        ...
        switch cn
            ...
            case 'acc_c_wes'
                b = ESG.N2;
                if isfield(b,'effectsize')
                    a = b.effectsize;
                else
                    a = NaN;
                end
            case 'acc_c_mes'
                b = ESG.(strainnames{si});
                if isfield(b,'effectsize')
                    a = b.effectsize;
                else
                    a = NaN;
            ...
            case 'acc_c_ph_w4m4'
                ...
                a = PHC.pValue(i);
            case 'acc_c_ph_m0m4'
                i = ismember(PHC.factors_1,[sn,'*0mM']) & ismember(PHC.factors_2,[sn,'*400mM']) | ...
                    ismember(PHC.factors_2,[sn,'*0mM']) & ismember(PHC.factors_1,[sn,'*400mM']);
                a = PHC.pValue(i);
            case 'acc_c_spw_w0w4'
            ...
            case 'acc_c_spw_m0m4'
                a = timecomppertap(PHCbytap,[sn,'*0mM'],[sn,'*400mM']);
...
for si = 1:size(TALL,1)
    ...
    % - mutant effect
    mut_ttest = TALL.curve_mt_pct_ttest(si);
...
phenotype = {'intermediate','-';...
            ...
            'repeat experiment','R';...
            'ko - no interaction','x*' ;...
            'intermediate - no interaction','-*'};
% curve
c = TALL.curve_decision;
[i,j] = ismember(c,phenotype(:,1));
a = phenotype(j,2);
TALL.curve = a;
% reversal
r = TALL.reversal_decision;
[i,j] = ismember(r,phenotype(:,1));
a = phenotype(j,2);
TALL.rev = a;
% acc
a = TALL.acceleration_decision;
[i,j] = ismember(a,phenotype(:,1));
a = phenotype(j,2);
TALL.acc = a;
