...
for ti = 1:numel(timelist)
    ...
    for si = 48%1:size(strainNames,1) % cycle through strains
...
%         readtable(sprintf('%s/%s graph values.csv',pData,strain));
    ...
    %         GraphData.N2_x <= responseT2);
        ...
        if ~isempty(tv)
            ...
            fprintf(' %s',s)
            fprintf(', suggesting %s altered %s',genotype, phenotypeName);
        else
            fprintf('was similar to the wildetype 0mM group');
            fprintf(', suggesting %s had no effect on %s', genotype, phenotypeName)
        end
        ...
        if ~isempty(tv)
            ...
            s = taptime_pvalue(tv,pstr);
            fprintf(' %s',s)
            fprintf(', suggesting ethanol altered %s. ', phenotypeName);
        else
            fprintf('was similar to the wildtype 0mM group');
            fprintf(', suggesting ethanol failed to alter %s', phenotypeName);
        end
        % ----------------------------------------------------------------

        
        %% mutant 0mM vs mutant 400m -------------------------------
        fprintf('%s of %s 400mM group ',phenotypeName, genotype);
        % compare between groups
