...
for si = 27%1:size(strainNames,1)
    ...
    k= [find(i); find(~i)];
    n_sample = n_sample(k);
    fprintf(fid, ['(N=',strjoin(num2cellstr(n_sample)',','),'), ']);
    ...
    %% wildtype -----------------------------------------------------------
    fprintf(fid,'Wildtype 0mM and 400mM group showed ');
    ...
    if p < alpha
        ...
        fprintf(fid,'significantly different velocity curve (%s). ',pstr);
    else
        fprintf(fid,'similar velocity curve (%s), ',pstr);
        fprintf(fid,'suggesting ethanol treatment may have failed. ');
    end
    
    %% wildtype 0mM pairwise time
    fprintf(fid,'Velocity of wildtype 0mM group ');
    %% get data
    ...
    if any(pvs)
        fprintf(fid, 'significantly decreased after the tap ');
        if sum(pvs) == numel(response) && all(pv < pvlimit)
            fprintf('(%.1fs-%.0fs < baseline, p<%.3f), ',responseT1,responseT2,pvlimit);
        end
    end
