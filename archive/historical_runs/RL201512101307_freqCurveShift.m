...
%% get speed data
% find initial mean
a1 = mean(MWTSet.Data.ByGroupPerPlate.N2.([msrname,'_Mean'])(1,:));
a2 = mean(MWTSet.Data.ByGroupPerPlate.N2_400mM.([msrname,'_Mean'])(1,:));
% find difference between N2 inital mean and N2_400mM initial mean
df = a2-a1;
%% see if still RMANOVA diff
pSaveA = [pA,'/Stats HabCurve Norm']; if isdir(pSaveA) == 0;mkdir(pSaveA); end

A = {};
A{1} = MWTSet.Data.ByGroupPerPlate.N2.([msrname,'_Mean'])';
A{2} = (MWTSet.Data.ByGroupPerPlate.N2_400mM.([msrname,'_Mean']) - df)';
...
y = [mean(A{1})' mean(A{2})'];
errorbar(y,e)
savefigeps(sprintf('%s Normalized',msrname),pSaveA);