...
for si = 1:size(strainNames,1);
    ...
    %% acc graph ++++++++++++
    TWR = rmANOVATWR;
    TWR.datapath = fullfile(pData_Strain,strain,'TAR/Dance_rType/AccProb RMANOVA.txt');
    ANOVA = TWR.anova;
    ...
    Pacc.p_strain(si) = ANOVA.pvalue('strain');
    Pacc.p_dose(si) = ANOVA.pvalue('dose');
    Pacc.p_strain_dose(si) = ANOVA.pvalue('dose*strain');
