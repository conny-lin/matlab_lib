...
%     gnlist = gnlist([find(i); find(~i)]);
    ...
    for gi = 1:numel(gnlist) % cycle through groups
        ...
        for platei = 1:numel(plateu)
            ...
            if size(Baseline,1) < n_lowest
               ...
               fprintf('skip plate N<%d: %s %s %s',n_lowest, ...
                   info.mwtname, info.expter, info.groupname);
               return
            end
            ...
            % run function 
            [Rsum,~,~,~] = compute_response_type(Response, Baseline, 'pSave',pM);
            % ----------------------------------
            
            % put output
            Output.(gn).(['mwtid',num2str(mwtid)]).RSwitchSum = Rsum;
        end
    end
    ...
    % create table summary
            Rsum.VariableNames = {'response',[gn,'_n'], [gn,'_p']};
            if platei ==1
               Tsum = Rsum;
            else
               Tsum = outjoin(Tsum, Rsum);
            end
            return
