...
addpath(pM);
pData = '/Users/connylin/Dropbox/RL Pub PhD Dissertation/Chapters/4-EARS/3-Results/2-Genes EARS/Summary v3/ephys_accpeak_graph3_bm';
...
for si = 1:size(strainNames,1)
    ...
    if any(S.pValue(2:end) < alpha)
        ...
        fprintf('An rANOVA evaluating the effect of %s and dose on response velocity to the last 3 taps revealed significant effect of ', genotype);
        % anova strain
        rowName = 'strain:t';
...
%                 mut_vs_wt_d = false;
    ...
    if pvalue < alpha
        ...
        fprintf('did not differ from the wildtype 0mM group (curve, %s)',pstr);
    ...
    if ~wtEARS_failed 
        if pvalue < alpha
            ...
            fprintf(', indicating ethanol did not have effect on wildtype');
        end
    else
        fprintf(', indicating wildtype failed to show EARS');
        if pvalue < alpha
            fprintf(' but ethanol had effect on wildtype velocity response');
        else
            fprintf(' and ethanol had no effect on wildtype velocity response');
        end
...
%     fprintf('Velocity curve of the %s 400mM group ', genotype);
    ...
    if pvalue < alpha
        pstr = print_pvalue(pvalue,pvlimit,alpha);
        fprintf(', and was different from the %s 0mM group (curve, %s), ',genotype, pstr);
        fprintf('indicating %s ', genotype);
        if mutEars
            fprintf('retained EARS and the effect of ethanol on response velocity');
        else
            fprintf('disrupted EARS but retained some effects of ethanol on response velocity');
        end
    else
        fprintf('was not different from the %s 0mM group, ',genotype);
        if ~mutEars
            fprintf('indicating %s disrupted EARS and the effect of ethanol on response velocity', genotype, genotype);
        else
            fprintf('indicating %s retained EARS but disrupted some effects of ethanol on response velocity', genotype);
