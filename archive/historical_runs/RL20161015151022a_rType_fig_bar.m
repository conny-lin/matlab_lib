...
for ti = 1:numel(timelist) % cycle through time
    ...
    timename = timelist{ti};
    strainlist = dircontent(fullfile(pData,timename),'*.mat');
    ...
    for si =1:numel(strainlist)
        ...
        p =fullfile(pData,timename,[strain,'.mat']);
        load(p);
        % -----------------------------------------------
        ...
        svname = [strain, 'by plate.csv'];
        writetable(T3,svname);
        % ---------------------------------------

        %  summarize by group --------------
        statsBasic(T3.pct,T3.groupname)
        return
        % ---------------------------------------
        
        % get data for figure ---------------------
%         t = repmat(time,size(Mean,1),1);
        ...
        % Create figure ----------------------
        figure1 = figure('Visible','off','PaperUnits','inches','PaperPosition',[0 0 w h]);
        % Create axes
