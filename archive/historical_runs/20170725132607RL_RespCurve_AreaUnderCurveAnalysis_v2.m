...
for si = 1:numel(strainnames)
    ...
    if exist(pdata,'file')
        ...
        for msri = 1:numel(msrlist) 
            ...
            [txt,anovastats,multstats,T2,ST] = anova1_std_v2(T.(msr),T.groupname, 'plate');
            % put in central array
            SS.(msr).raw = T;
            SS.(msr).anovatext = txt;
            SS.(msr).anovastats = ST;
        ...
        % ttest against zero
        % open text file
        fid = fopen(fullfile(pSave, sprintf('%s ttest against 0.txt',caltype)),'w'); % open file
