...
% get mwt paths
pData = fullfile(fileparts(pSave),'Data_201706281236','Dance_ShaneSpark_v1707','Dance_ShaneSpark_v1707');
load(pData);
MWTDB = MWTSet.MWTDB;
...
%% data prep
Data = innerjoin(MWTDB(:,{'mwtid','groupname','strain','rx'}),MWTSet.Raw);
...
%% bootstrap
y = Data_0mM.RevFreq;
[stats,bootsam] = bootstrap(N_plate,@(x) [mean(x) std(x)],y)
