...
%         easier to use raw data and analyze posthoc directly
        ...
        elseif ismember(cn,'rev_c_spw_w0w4') 
            clear D;
            switch anamec
                case 'Dance_ShaneSpark4'
                    Raw = MWTSet.Raw;
                    DD = outerjoin(Raw, MWTSet.MWTDB(:,{'mwtid','groupname'}));

                    
                otherwise
                    D = MWTSet.Stats.RevFreq.Curve.posthoc.factors_by_tap;  
            end
            % process pairwise tap
            switch cn
                case 'rev_c_spw_w0w4' % pairwise taps
                    i = ismember(D.factors_1,{'N2','N2*0'}) ...
                        & ismember(g2,{'N2_400mM','N2*400'});
                    p = D.pValue(i);
                    k = p < 0.05;
                    t = D.tap(i(k));
                    a = strjoin(t',',');
