...
for si =1:numel(strainlist)
    ...
    for ti = 1:numel(timeNameList)
        ...
        sprintf('RMANOVA(tap:%s):\n%s\n',factorName,...
            anovan_textresult(ranova(rm), 0, 'pvlimit',pvlimit))

        % run pairwise
        astring = sprintf('%s1-%s20~%w',rmfactorName,rmfactorName,factor1Name);
        rm = fitrm(D,astring,'WithinDesign',tptable);
...
function graphEy(G,strain,timeName,pSave,Setting)
