...
for si = 1:size(SampleSize,1)
    ...
    T.gn = GNC(:,1);
    T.n = cell2mat(GNC(:,2));
    T = sortN2first(T.gn,T);
    nstr1 = strjoin(num2cellstr(T.n'),',');
    SampleSize.N_in{si} = nstr1;

    % get actual N from reversal analysis
    p = fullfile(fullfile(pData_Strain,strainname,'TWR','Dance_ShaneSpark4','Dance_shaneSpark4.mat'));
    load(p,'MWTSet');
    n = MWTSet.ByGroupPerPlate.RevFreq.N(1,:);
    a = num2cellstr(n);
    nstr2 = strjoin(a,',');
    SampleSize.N{si} = nstr2;
    clear a b n
    
    if ~isequal(nstr1, nstr2)
        SampleSize.Neq{si} = '*';
...
writetable(SampleSize,fullfile(pM,'N table.csv'));
