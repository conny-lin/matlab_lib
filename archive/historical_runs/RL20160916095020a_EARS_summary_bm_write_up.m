...
pSampleSize = '/Users/connylin/Dropbox/RL Pub PhD Dissertation/4-STH genes/3-EARS/Data/ephys_accpeak_samplesize';
...
for si = 10%1:size(strainNames,1)
    %% get strain
    ...
    %% sample size
    fullfile(pSampleSize, [strain, ' ephys t28-30 samplesize.csv'])
    ...
    rtime = GraphData.N2_x(GraphData.N2_x >= responseT1 & GraphData.N2_x <= responseT2);
    ...
    rval = response_lower;
    [pstr,pvt,tv] = pvaluestring(rtime,rval,pv,alpha,pvlimit);
    if any(tv)
...
%     end
    ...
    if p < alpha
        pstr = print_pvalue(pvalue,pvlimit,alpha);
        fprintf('was significantly different from that of wildtype 0mM (%s), ',pstr);
    ...
    if p < alpha
        ...
        fprintf('was similar to that of wildtype 400mM (%s), ',pstr);
