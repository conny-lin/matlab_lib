...
for si = 9%1:size(strainNames,1)
    ...
    fprintf('Velocity of %s group ', gnametext);
    % wildtype 400mM pairwise time
    [str,passed] = taptime_result(GraphData,ST,gname,'increased');
    if passed
        fprintf('%s, suggesting ethanol treatment may have failed',str);
        wtEARS_failed = true;
    else
        fprintf('%s, indicating wildtype showed EARS',str);
        wtEARS_failed = false;
    end
    fprintf('. ');
