...
for ti = 1%:numel(timelist) % cycle through time choices
    ...
    for si = 46%1:size(strainNames,1) % cycle through strains
        ...
        [T1, RTclean, R, RN,legend] = compute_response_type(Response, Baseline, 'pSave',pM);
        R = cell2table(R);        
        % ---------------------------------
        ...
        % make figure
        figure1 = createfigure_rType(time', Mean', SEM', gnlist, genotype);
        % ------------------------------------------------
