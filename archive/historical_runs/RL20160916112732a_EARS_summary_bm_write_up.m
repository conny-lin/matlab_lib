...
for si = 1:size(strainNames,1)
    ...
    % determine
    conditions = [any(response_higher(2:5) == true)];
    if all(conditions)
   ...
   %% mutant 400mM pairwise time
    ...
    % get data
    fn = [gname,'_y'];
    [baseline, response, response_lower, response_rev, response_higher] = response_data(GraphData,fn, xtime, rtime, baselinetime, baseline);
    [pv,tr] = get_pvalue(ST,10,gname);
    ...
    % determine
    conditions = [any(response_higher(1:5) == true)];
    if all(conditions)
        fprintf(', indicating %s showed EARS', gnametext);
    else
        fprintf(', but response velocity did not increase above baseline');
