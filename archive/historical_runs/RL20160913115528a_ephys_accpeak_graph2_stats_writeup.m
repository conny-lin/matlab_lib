...
%% title sentence
fprintf('For %s %s allele, ',genotype, mutation_type);
if ~isempty(strinsig)
...
% fullfile(pM,'/Users/connylin/Dropbox/RL Pub PhD Dissertation/4-STH genes/Data/10sIS by strains/%s/ephys graph/data_ephys_t28_30.mat',strain
...
n = nan(size(DataG,2),1);
for gi = 1:size(DataG,2)
    n(gi) = size(DataG(gi).speedb,1);
end
fprintf(['(N=',strjoin(num2cellstr(n)',','),').']);
fprintf(' ');
