...
pGraphNew = '/Users/connylin/Dropbox/Publication/Manuscript RL Alcohol Genes/Figures Tables Data/Data new/Graph';
pGraphOld = '/Users/connylin/Dropbox/Publication/Manuscript RL Alcohol Genes/Figures Tables Data/Tbl2 genetic screen result summary/Thesis/Summary/SummaryAssemble1/Graphs';
pStatsOld = '/Users/connylin/Dropbox/Publication/Manuscript RL Alcohol Genes/Figures Tables Data/Tbl2 genetic screen result summary/Thesis/Summary/SummaryStatsReport_201610241113/Report';
pStatsNew = '/Users/connylin/Dropbox/Publication/Manuscript RL Alcohol Genes/Figures Tables Data/Data new/Stats';
pSumDataOld = '/Users/connylin/Dropbox/Publication/Manuscript RL Alcohol Genes/Figures Tables Data/Tbl2 genetic screen result summary/Thesis/Summary/SummaryStatsReport_201610241113/Report';
        ...
        if numel(unique(groupname))==1
            ...
            for si = 1:numel(strainsU)
                ...
                if ~isempty(olddata)
                    ...
                    pd = create_savefolder(pM,fullfile('Data',groupname,genename,s));
                    psd = char(olddata);
                    [~,psc] = dircontent(psd);
                    if ~isempty(psc)
                ...
                if ~isempty(newdata)
                    % create strain and genotype folder (i.e. slo-1(eg142) JPS326 LOF
                    pd = create_savefolder(pM,fullfile('Data',groupname,genename,[s,' New']));
                    psd = char(newdata);
                    [~,psc] = dircontent(psd);
                    if ~isempty(psc)
                        cellfun(@movefile,psc,cellfunexpr(psc,pd),cellfunexpr(psc,'f'));
...
% get group names
[fn,p] = dircontent(fullfile(pM,'Data'));
...
for si = 1:numel(strainnames)
    ...
    % move data file if old
    if isempty(newstatus) % if new folder
        ps = fullfile(pSumDataOld,sprintf('%s.mat',s)); % get source file path
        movefile(ps,pd); % move file
    end
