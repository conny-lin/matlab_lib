...
for si = 15:numel(strainnames) % for each strain
    % report loop
    ...
    % search database
    [pMWT,MWTDB]  = search_MWTDB('expname',e,'groupname',{'N2','N2_400mM',strain,g},'rc','100s30x10s10s');
    %% check if has n2
    if ~any(ismember(MWTDB.groupname,{'N2'}))
        en = numel(e);
        % select nearby N2
        ed1 = max(MWTDB.exp_date)+ 3;
        ed2 = min(MWTDB.exp_date)- 3;
        ...
        e2 = [table2cell(DBe.expname(1:en)); e];
        [pMWT,MWTDB]  = search_MWTDB('expname',e2,'groupname',{'N2','N2_400mM',strain,g},'rc','100s30x10s10s');
