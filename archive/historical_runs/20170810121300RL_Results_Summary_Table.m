...
coltype = {'cell','double','double','double','double'};
    ...
    for ci = 1:numel(colnames)
        ...
        switch cn
            ...
            case 'rev_mut_area_ttest'
                ...
                a = sprintf('%s_400mM',a);
                % get relevant number
                p = MWTSet.area_pctctrl.RevFreq.ttest_against0.(a).p;
                % put in table
                T.(cn)(si) = p;
            case 'rev_mtmut_anova'
                % get strain field name
                a = strainnames{si};
                a = regexprep(a,' New','');
                a = sprintf('%s_400mM',a);
                % get relevant number
                p = MWTSet.area_pctctrl.RevFreq.anovastats.p('Groups');
                % put in table
                T.(cn)(si) = p;
        end
    end
end
% put in overall table
TALL = outerjoin(TALL,T,'Key','strain','MergeKey',1);
% ----------------------------------------------------------------20170725-
