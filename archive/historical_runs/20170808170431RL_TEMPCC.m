...
for si = 1:numel(strainNew)
    ...
    if sum(i) == 0 % if no old data, 
       ...
       pOld = pstrains{i};
       % find fn
       fn = dircontent(pStrainFNew);
       % create new paths to old folder
       a = cellfunexpr(fn,pOld);
       pNewcontent = cellfun(@fullfile,a,fn,'UniformOutput',0);
       ...
       % get file paths
       [~,ps] = dircontent(pStrainFNew);
       pd = regexprep(ps,pStrainFNew,pOld); % get des
