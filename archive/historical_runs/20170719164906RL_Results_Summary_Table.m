...
% get group names
[groupnames,p] = dircontent(pData);
% minus archive code folder
p(regexpcellout(groupnames,'[_]')) = [];
% get gene names
[genenames,p] = cellfun(@dircontent,p,'UniformOutput',0);
p = celltakeout(p);
genenames = celltakeout(genenames);
% get strain names
...
colnames = {'group','gene','strain','rev_N','rev_wt_sig'};
T = table;
T.strain = strainnames;
T.group = groupnames;
T.gene = genenames;
for ci = 4:numel(colnames)
