...
% construct report
txt = sprintf('\n\nTo see if %s affects ethanol modulated increase',genotype);
...
if pv < pvsig
    s = 'produced the expected elevation of acceleration response probability in';
    s1 = sprintf('wildtype (%s)',char(D.ptxt(compname)));
    s = sprintf('%s %s',s,s1);
    wtsig = true;
else
    s = 'did not produce the expected elevation of acceleration response probability in wildtype';
    wtsig = false;
end
txt = sprintf('%s %s',txt,s);
...
D = A.posthoc_groups;
compname = sprintf('%s_*%s_400mM',strain, strain);
pv = D.pvalue(compname);
pvt = char(D.ptxt(compname));
if pv < pvsig
    if wtsig
        s = sprintf(', and the %s mutant (%s)', genotype,pvt);
    else
        s = sprintf(', but did for the %s mutant(%s)', genotype,pvt);
    end
else
    if wtsig
