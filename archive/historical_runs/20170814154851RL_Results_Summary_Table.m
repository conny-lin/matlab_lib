...
%% MAKE INFO TABLE: CURVE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%20170814
% make empty table
T = table;
T.strain = strainnames;
...
for si = 1:numel(strainnames)
    ...
    T.curve_file_source{si} = aname; % record source file name
...
%     PHCbytap = MWTSet.Stats.RevFreq.Curve.posthoc.factors_by_tap;
    ...
    for ci = 2:numel(colnames)
        ...
        switch cn
            ...
            case 'curve_mt_pct'
                a = PCT(ismember(PCT.group,[sn,'_400mM']));
            case 'curve_wt_pct_ttest'
                a = TTEST(ismember(TTEST.group,'N2_400mM'));

            case 'curve_m_pct_ttest'
                a = TTEST(ismember(TTEST.group,[sn,'_400mM']));
