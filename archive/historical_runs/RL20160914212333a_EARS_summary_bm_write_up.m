...
for si = 1%:size(strainNames,1)
    ...
    if any(pvs)
        ...
        % find if in sequnce
       dif = [0;diff(response_lower)];
       r = find(response_lower);
       r1 = r(1);
       rf = r(end);
       if any(dif(r1:rf)~=0)
           a = num2cellstr(response_time(response_lower));
    ...
    rvalid = response_higher;
    [pstr,pvt,pvs,pvst] = pvaluestring(pv,rvalid,alpha,pvlimit); 
    if any(pvs)
       fprintf('significantly increased '); 
       taptime_pvalue(rvalid,rtime,pstr);
