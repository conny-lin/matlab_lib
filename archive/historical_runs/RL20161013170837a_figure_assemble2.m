...
%     for ti = 1:numel(tnamelist)
    ...
    Header = repmat(255,100,ncol,3);
    % ----------------------------
    % add bottom space -----------
    [nrow,ncol,nlayer] = size(IMMH);
    IMmid = repmat(255,150,ncol,3);
    % ---------------------------
...
p = fullfile(pDataLine, tname,strain,[gn,'.tif']);
I = imread(p);
imshow(I)
...
return
    ...
    printfig(savename,pM,'w',5,'h',5,'closefig',0)
    % -------------------------------------------------
