...
%     size(a)
    ...
    for gi = 1:numel(gnlist) % cycle through groups
        ...
        D = Output.(gn);
        pinfo = D.plateinfo;
        mwtu = unique(pinfo.mwtid);
        R = D.RespType;
        R = regexprep(table2cell(R),'','no data');
        
        for pi = 1:numel(mwtu)
            
            R1 = R(pinfo.mwtid == mwtu(pi),:);
            R1 = categorical(R1);
            a = countcats(R1);
