...
%% see which lifestyle components come together
%% prep data +++++++++++
a = {'age','gender','education'};
b = regexprep(legend_LS_subsection.subsection(2:end),' ','_');
categories = [a b'];
% combine data
...
Data = [U LS_Score_subsection(:,2:end)];
% remove age = nan
i = isnan(User.age);
Data(i,:) = [];
% User(i,:) = [];
...
titlename= '';
biplot(coefforth(:,1:2),'scores',score(:,1:2),'varlabels',var);
title(titlename)
savefig(fullfile(pM,['pca plot ',titlename]));
