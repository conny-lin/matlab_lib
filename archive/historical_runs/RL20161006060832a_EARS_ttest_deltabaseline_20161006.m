...
DF = nan(size(strainNames,2),size(dS,2)*numel(groupnames));
TStatR = DF;
TStatL = DF;
TailRp = DF;
TailLp = DF;
...
for si = 36:size(strainNames,2)
    ...
    for gi = 1:numel(groupnames); % cycle through groups
        ...
        for ti = 1:size(dS,2)
            ...
            DF(r1:r2,ti) = stats.df;
            TStatR(r1:r2,ti) = stats.tstat;
            TailRp(r1:r2,ti) = p;
            
            [~,p,~,stats] = ttest(dS(:,ti),0,'Tail','left');
            TStatL(r1:r2,ti) = stats.tstat;
            TailLp(r1:r2,ti) = p;
        end
    end
