...
if AtimeMax < DBtimeMax
    ...
    % get rid of N2s
    strainNewData(regexpcellout(strainNewData,'N2')) = [];
end
a = numel(strainNewData);
fprintf('%d strains\n',a);
...
for si = 11:numel(strainNewData) % for each strain
    % report loop
    loopreporter(si,'strain',1,numel(strainNewData));
    strain = strainNewData{si}; % strain name
    % get pSave
    i = ismember(strainnames,strain);
    if sum(i) == 1
        pSaveStrain = pstrains{i};
    end
    % get pMWT ----------
    % create group name
    g = [strain,'_400mM'];    
