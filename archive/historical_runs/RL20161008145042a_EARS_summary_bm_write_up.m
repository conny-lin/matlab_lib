...
for si = 1:size(strainNames,1)
    ...
    gname = 'N2';
    % pairwise
    fprintf('%s',taptime_result(GraphData,ST,gname,'decreased below baseline'));
    fprintf(', and %s',taptime_result(GraphData,ST,gname,'decreased below zero'));
    ...
    gname = strain; 
    [str,passed] = taptime_result(GraphData,ST,gname,'decreased below baseline');
    if passed; fprintf('%s',str); end
    [str,passed] = taptime_result(GraphData,ST,gname,'decreased below zero');
    if passed; fprintf(', and %s',str); end
    [str,passed] = taptime_result(GraphData,ST,gname,'increased above baseline');
    ...
    fprintf('Velocity curve of the %s 0mM group', genotype);
    ...
    % title sentence
    fprintf('Velocity curve of the %s group ', gnametext);
