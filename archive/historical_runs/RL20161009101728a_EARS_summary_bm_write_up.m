...
%                 mut_vs_wt_d = false;
    ...
    if pvalue2 < alpha
        M4W4same = false;
        pstr = print_pvalue(pvalue,pvlimit,alpha);
        fprintf('was different from the wildtype 0mM group (curve, %s)',pstr);

    else
        M4W4same = true;
        fprintf('was not different from the wildtype 400mM group');
    end
    ...
    if mutEars
        if ~M4W4same
            fprintf('retained EARS');
        else
            fprintf('had wildtype EARS');
        end
    else
        fprintf('disrupted EARS');
        if M4W4same
    ...
    fprintf(' ');
    if M0M4same
        fprintf('and eliminated ');
    else
        fprintf(' but retained some ')  
    end
